 ...
 * Telegram update requests. This annotation provides a way to associate methods in a {@link
 * TelegramController} class with incoming Telegram updates based on their patterns, request types,
 * and message types.
 * <p>
...
public @interface TelegramRequest {
   ...
   * that will trigger the annotated method. This parameter is only applied if the {@link
   * #requestType()} is set to {@link RequestType#MESSAGE} or is not specified.
   *
   ...
   * accept any message type specified in the {@link #messageType()} parameter. If set to {@code
   * true}, the method will only accept messages that have all the types listed in the {@link
