...
public class DefaultScenarioChoiceTransitionConfigurer<S> implements ScenarioChoiceTransitionConfigurer<S> {

    private final ScenarioBuilder<S> builder;
    private final ChoiceTransitionConfigurer<S, ScenarioEvent> configurer;
    ...
    public DefaultScenarioChoiceTransitionConfigurer(
        ScenarioBuilder<S> builder, ChoiceTransitionConfigurer<S, ScenarioEvent> configurer
    ) {
    ...
    @Override
    public ScenarioChoiceTransitionConfigurer<S> source(S source) {
        this.source = source;
        return this;
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> first(S target, Guard<S> guard) {
        return this.first(target, guard, null, null);
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> first(S target, Guard<S> guard, Action<S> action) {
        return this.first(target, guard, action, null);
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> first(S target, Guard<S> guard, Action<S> action, Action<S> error) {
        this.first = new ChoiceData<>(source, target, guard, action, error);
        return this;
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> then(S target, Guard<S> guard) {
        return this.then(target, guard, null, null);
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> then(S target, Guard<S> guard, Action<S> action) {
        return this.then(target, guard, action, null);
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> then(S target, Guard<S> guard, Action<S> action, Action<S> error) {
        this.then.add(new ChoiceData<>(source, target, guard, action, error));
        return this;
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> last(S target) {
        return this.last(target, null, null);
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> last(S target, Action<S> action) {
        return this.last(target, action, null);
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> last(S target, Action<S> action, Action<S> error) {
        this.last = new ChoiceData<>(source, target, null, action, error);
        return this;
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> props(Map<String, Object> props) {
        this.props.putAll(props);
        return this;
    }

    @Override
    public ScenarioChoiceTransitionConfigurer<S> inlineKeyboardCreation() {
